...
class BleJmbMain:
    def __init__(self, ble, name="jmb-main-default"):
        self._ble = ble
...
def demo():
    ble = bluetooth.BLE()
    jmb_main = BleJmbMain(ble, 'jmb-main')

    def on_rx(v):
        data = bytes(v).decode('utf-8').split(' ')
#         print(data)
        msg = 'temp:' + data[0] + '°C hum:' + data[1] + '% pres:' + data[2] + 'hPa'
        print(msg, '\n')
#         print("RX", bytes(v).decode('utf-8'))

    jmb_main.on_write(on_rx)
    while True:
        pass
    time.sleep_ms(100)
...
#     i = 0
#     while True:
#         if jmb_main.is_connected():
#             # Short burst of queued notifications.
#             for _ in range(3):
#                 data = str(i) + "_"
#                 print("TX", data)
#                 jmb_main.send(data)5
#                 i += 1
#         time.sleep_ms(1000)
